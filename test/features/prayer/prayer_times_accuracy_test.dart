...
void _expect(DateTime actual, DateTime expectedUtc, String label) {
  ...
  expect(
    ...
    reason:
        '$label: got ${actual.toUtc().toIso8601String()}, '
...
void main() {
  ...
  group('Mecca, Saudi Arabia — Umm al-Qura (UTC+3)', () {
    ...
    test('Isha = Maghrib + 90 min (Umm al-Qura fixed offset)', () {
      ...
      expect(
        diff,
        closeTo(90, 2),
        reason: 'Umm al-Qura always uses 90-min Isha offset',
      );
    });
  });
  ...
  group('Cross-location invariants', () {
    test(
      'Hanafi Asr-from-Dhuhr gap > standard (Shafi\'i) gap at comparable latitudes',
      () {
        ...
        final kazan = service.calculate(
          latitude: 55.7879,
          longitude: 49.1233,
          date: _kDate,
        );
        final newYork = service.calculate(
          latitude: 40.7128,
          longitude: -74.0060,
          date: _kDate,
        );
        ...
        expect(
          ...
          greaterThan(nyGap),
          reason:
              'Hanafi Asr (shadow x2) should be further from Dhuhr than ISNA standard',
        );
      },
    );
    ...
    test(
      'Diyanet (Turkey) Isha-Maghrib gap is twilight-based, in range 60-180 min',
      () {
        final istanbul = service.calculate(
          latitude: 41.0082,
          longitude: 28.9784,
          date: _kDate,
        );
    ...
    test('All 6 prayers are strictly ordered at every tested location', () {
      ...
      for (final (lat, lng, city) in locations) {
        final p = service.calculate(
          latitude: lat,
          longitude: lng,
          date: _kDate,
        );
        expect(
          p.fajr.isBefore(p.sunrise),
          isTrue,
          reason: '$city: fajr < sunrise',
        );
        expect(
          p.sunrise.isBefore(p.dhuhr),
          isTrue,
          reason: '$city: sunrise < dhuhr',
        );
        expect(p.dhuhr.isBefore(p.asr), isTrue, reason: '$city: dhuhr < asr');
        expect(
          p.asr.isBefore(p.maghrib),
          isTrue,
          reason: '$city: asr < maghrib',
        );
        expect(
          p.maghrib.isBefore(p.isha),
          isTrue,
          reason: '$city: maghrib < isha',
        );
      }
    });
  });
